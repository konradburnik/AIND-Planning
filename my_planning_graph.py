...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        """
        # Make sure the graph should only be built during class construction
        if (len(self.s_levels) != 0) or (len(self.a_levels) != 0):
            raise Exception(
                'Planning Graph already created; construct a new planning graph for each new state in the planning sequence')

        # First we initialize the state S0 to all the literals
        ...
        # Next, we build up the rest of the levels.
        done = False
        while not done:
            ...
            if self.s_levels[level] == self.s_levels[prev_level]:
                done = True
    ...
    def add_literal_level(self, level):
        ...
        prev_action_level = self.a_levels[level - 1]

        for action_node in prev_action_level:
    ...
    def update_a_mutex(self, nodeset):
        ...
        # Check all unordered pairs of nodes
        for idx, n1 in enumerate(nodelist[:-1]):
            for n2 in nodelist[idx + 1:]:
                if (self.serialize_actions(n1, n2) or
                        self.competing_needs_mutex(n1, n2) or
                        self.interference_mutex(n1, n2) or
                        self.inconsistent_effects_mutex(n1, n2)):
                    mutexify(n1, n2)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check a pair of actions for inconsistent effects, whether
        # one action negates an effect of the other, we check both directions
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Separately process positive and negative preconditions.
        for effect in node_a1.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check if any parent's precondition is mutex of another parent precondition
    ...
    def update_s_mutex(self, nodeset: set):
        ...
        # Check all unordered proper pairs of nodes.
        for idx, n1 in enumerate(nodelist[:-1]):
            for n2 in nodelist[idx + 1:]:
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        # Build up the count level-by-level.
        level_sum = 0
        n = len(self.s_levels)
        for goal in self.problem.goal:
            ...
            for level in range(n):
                for state in self.s_levels[level]:
                    literal = expr('~{}'.format(state.symbol)) if not state.is_pos else expr(state.symbol)
                    if goal == literal:
                        goal_state_found  = True
                        level_sum += level
                        break
                if goal_state_found: break
        return level_sum
