...
class AirCargoProblem(Problem):
            ...
            for a in self.airports:
                for c in self.cargos:
                    for p in self.planes:
                        ...
                        precond_neg = []
                        result_add = [expr("In({}, {})".format(c, p))]
                        result_rem = [expr("At({}, {})".format(c, a))]
                        action = Action(expr("Load({}, {}, {})".format(c, p, a)),
                            [precond_pos, precond_neg], [result_add, result_rem])
                        loads.append(action)
            ...
            for a in self.airports:
                for c in self.cargos:
                    for p in self.planes:
                        ...
                        precond_neg = []
                        result_add = [expr("At({}, {})".format(c, a))]
                        result_rem = [expr("In({}, {})".format(c, p))]
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        executed.
        """
        # Based on Udacity forum discussion https://discussions.udacity.com/t/understanding-ignore-precondition-heuristic/225906/2
        kb = PropKB()
...
def air_cargo_p1() -> AirCargoProblem:
    # Implement Problem 1 definition
    cargos = ['C1', 'C2']
    planes = ['P1', 'P2']
    airports = ['JFK', 'SFO']
    pos = [expr('At(C1, SFO)'),
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), expr('At(C1, ATL)'), expr('At(C1, ORD)'),
           ...
           expr('At(C4, JFK)'), expr('At(C4, SFO)'), expr('At(C3, ATL)'),
